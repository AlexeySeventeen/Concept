<template>
  <div class="mt-3 relative">
    ...
    <div>
      <button class="absolute top-[-30px] right-[-20px]" @click="$store.commit('changeFavorite', !this.$store.state.currentProject.isFavorite )">
        <svg
          :class="{'fill-[red]': this.$store.state.currentProject.isFavorite}"
          width="28"
          height="28"
          viewBox="0 0 24 24"
          fill="white"
      ...
      <button class="absolute top-[-31px] right-[14px] w-6 h-5" @click="this.$store.commit('changeArchive', !this.$store.state.currentProject.isArchive)">
        <svg
    ...
    <h1
      ...
      placeholder="text"
      class="ml-[26px] text-3xl font-medium font-segoe outline-0 after:content-['Untitled'] after:text-text-passive">
